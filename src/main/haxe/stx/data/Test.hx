...
enum StoreValOrContents{
  IsStoreVal;
  IsContent;
}

typedef StoreKind<T>     = stx.fp.Kind<T,{ hashable : T -> Hash }>;
typedef Xi<T>            = StoreKind<T>;
...
class BlockStoreOne<K,V>{
  ...
  function obtain(k:Articulation<K>):Array<HashedArrayOfEntry<K>>{
    return k.toIter().foldl(
      (key:K,memo:Array<HashedArrayOfEntry<K>>) -> {
        return memo.last().flat_map(
          ent -> (ent.snd()).search(
            (entry) -> entry.key == key
          ).flat_map(
            ...
            (x:Couple<Option<Hash>,EntryArray<K>>) -> memo.snoc(x)
        ).defv(memo);
      },
  ...
  **/
  public function set(k:Articulation<K>,v:V):Execute<DbFailure>{
    ...
    var src         = obtain(k);
    trace('path: ${src.map(x -> x.show())}');
    var have        = $type(src.map(Some).pad(k.length,None));
    var want        = k.zip(have);
    trace("__");
    var next        = want.rfold(
      (next:Couple<K,Option<HashedArrayOfEntry<K>>>,memo:Couple<Either<Hash,Hash>,Array<HashedArrayOfEntry<K>>>) -> {
        ...
        return branched.if_else(
          ...
          () -> {
            var n_src         = next.snd().defv(HashedArrayOfEntry.unit());
            trace(n_src);
            var n_next        = n_src.mod(
              (arr) -> arr.map(
                entryI -> (entryI.key == entry.key) ? entry : entryI
              )
            ).rehash();
            var n_hash        = Right(n_next.fst().fudge());
            var n_memo        = __.couple(n_hash,memo.snd().cons(n_next));
            return n_memo;
          }
        );
    ...
    head.set('master',next.snd().head().fudge().fst().fudge());
  ...
  public function get(k:Articulation<K>):Proceed<Option<V>,DbFailure>{
    ...
    var opt = ls.last().flat_map(x -> x.fst()).flat_map(
